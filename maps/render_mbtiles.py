...
ROAD_STYLE = {
    ...
    "unclassified": ((250, 250, 248), 2), "service": ((224, 223, 217), 1),
}
    ...
    for zoom in range(8, 14):
        ...
        for x in range(x0, x1):
            for y in range(y0, y1):
                ...
                roads = db.execute("""SELECT roads.kind,roads.name,roads.geometry FROM roads_rtree
                ...
                labelled = set()
                for kind, name, raw in roads:
                    color, base_width = ROAD_STYLE.get(kind, ((201, 200, 194), 1))
                    ...
                    if zoom >= 12 and name and kind in label_kinds and name not in labelled and len(points) > 1:
                        ...
                        if draw_label(draw, middle, name, (68, 67, 63), font, occupied):
                            labelled.add(name)
                minimum_kind = ("city", "town", "village") if zoom <= 10 else ("city", "town", "village", "suburb", "neighbourhood", "station")
                ...
                for name, kind, lat, lon in db.execute(
                    ...
                    px = ((project(lon, lat, zoom)[0] - x) * 256, (project(lon, lat, zoom)[1] - y) * 256)
                    draw_label(draw, px, name, (39, 56, 66), font, occupied)
                buffer = io.BytesIO()
