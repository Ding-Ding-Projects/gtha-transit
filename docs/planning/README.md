...
Each journey and each transit leg also carries a live-status chip -- early, on time, late, cancelled, timetable-only, or not-yet-matched -- refreshed by a 30-second poll of every leg still worth checking. See [live delays and status](live-delays.md) for the thresholds, the colours, and exactly which agencies' trip updates actually reach a plan.
...
Suggested articles: [live delays and status](live-delays.md), [TTC status](../status/README.md), [regional data](../data/README.md), [deployment](../deployment/README.md).
