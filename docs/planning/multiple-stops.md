...
A selected stop may also include a qualified `id`, `stopId`, or `stationId`. The server treats that value only as a lookup request. It sends OTP `visit.stopLocationIds` only after the local, date-valid published stop index resolves one exact identifier and confirms that the published stop is within 50 metres of the supplied coordinates. A source record is date-valid only when its own generated `serviceStart` and `serviceEnd` range contains the selected service date. Missing, unknown, stale, malformed, nonnumeric, or out-of-range source coordinates cannot create an anchor. When a retired identifier has one date-valid active alias, the active identifier is used; ambiguous aliases are rejected. The resolved active graph identifier is sent as a one-item list, so an exact platform is not silently broadened to a nearby station. Unknown, stale, malformed, mismatched, map-picked, or user-invented identifiers remain coordinate visits. The server keeps the user-supplied coordinates and label in all cases; it never moves a selected point to a different published stop.
...
The route response preserves official OpenTripPlanner identifiers without manufacturing identity from a place name or coordinate. When a leg endpoint or intermediate place includes `stop.gtfsId`, its normalized `id` and `stopId` retain that exact feed-qualified value. `stationId` retains an official parent-station identifier when supplied, or the same official identifier for a station record. `agencyFeedId` is derived only from the qualified identifier's feed prefix. A place with no official identifier remains without these identity fields. Each transit leg also keeps the exact qualified `routeId` and `routeGtfsId` from `route.gtfsId`, separately from the user-facing `route` label such as a short route name. This lets downstream validation compare published identifiers instead of display text.
