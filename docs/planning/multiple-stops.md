...
A selected stop may also include a qualified `id`, `stopId`, or `stationId`. The server treats that value only as a lookup request. It sends OTP `visit.stopLocationIds` only after the local, date-valid published stop index resolves one exact identifier and confirms that the published stop is within 50 metres of the supplied coordinates. The resolved active graph identifier is sent as a one-item list, so an exact platform is not silently broadened to a nearby station. Unknown, stale, malformed, mismatched, map-picked, or user-invented identifiers remain coordinate visits. The server keeps the user-supplied coordinates and label in all cases; it never moves a selected point to a different published stop.
