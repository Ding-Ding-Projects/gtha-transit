...
Official anchor stops travel to OTP as `visit.stopLocationIds`, not as ordinary coordinate visits. A coordinate visit can resolve by proximity and therefore cannot prove an exact platform identity. Using the publisher's exact stop identifier targets that stop, while the returned itinerary must still carry the same identifier before the visit is accepted. User-selected places without a verified stop identifier remain coordinate visits.
