...
Capture plans contain local operational paths and identifiers. Never commit them or raw diagnostics. The helper suppresses reflected transport error content and rejects credential-bearing page URLs. It cannot determine whether a page or PNG contains private user data; use an isolated public state and review every capture. A failed capture retains an incomplete record where possible; a failed record write reports failure and must never be treated as successful capture evidence. Failed partial files remain in the private run directory for explicit review.
