...
Settings has six sections: Appearance, Language, Comfort, Narrator, Schedule and Privacy. The selected section is kept in the browser under `gtha-settings-section-v1`. The tabs use the existing accessible tab primitive, with arrow-key navigation, labelled panels and a responsive strip. Changing sections does not recreate the planner's narration controller.

Appearance offers explicit Light and Dark choices, and also hosts the [app logo picker](appearance.md#app-logo-customization). Language offers English, Hong Kong Cantonese and bilingual presentation, followed by separate English and Cantonese playfulness sliders. Each slider keeps its five-level preview and can reset independently to the shipped level 5. Narration retains its own language, English and Cantonese voice choices, rate, pitch, quiet mode and preview. It remains off by default. [Schedule](scheduled-settings.md) adds time-of-day and weekday rules, in America/Toronto time, that automatically switch the language mode, theme or an appearance preset, plus a manual override and an external settings-document import. Privacy explains local preferences, routing requests, explicit sharing and independent service data.
