...
/**
 * A single stable empty map, returned while tracking is off so a masked
 * render never allocates a fresh one. Nothing that receives it ever writes
 * to it -- every consumer only reads through `Map#get` -- so sharing one
 * instance across every inactive render is safe.
 */
const EMPTY_LIVE_MAP: Map<string, LiveJourneyLeg> = new Map();
...
export function useLiveJourneys(journeys: Itinerary[], options: UseLiveJourneysOptions = {}): UseLiveJourneysResult {
  ...
  /**
   * `Date.now()` is impure, so it is read once as a lazy `useState`
   * initializer -- the one place React itself blesses calling something
   * impure to seed state -- rather than directly in the render body. The
   * tracking window (3 h ahead, 30 min of grace) does not need millisecond
   * freshness, so a minute-granularity tick is enough to keep a long-open tab
   * from freezing its notion of "now" at whenever it happened to mount.
   */
  ...
  useEffect(() => {
    // Nothing to reset here: `live`/`checkedAt`/`failed` are masked back to
    // their empty defaults below whenever tracking is inactive, so this
    // branch does not need its own setState calls to make that true.
    if (!enabled || !trackableKey) return;
  ...
  const tracking = enabled && Boolean(trackableKey);
  return {
    live: tracking ? live : EMPTY_LIVE_MAP,
    checkedAt: tracking ? checkedAt : null,
    failed: tracking ? failed : false,
    refreshNow,
  };
}
  ...
  return journeys.map((journey) => {
    ...
    const legs = journey.legs.map((leg) => {
      ...
      return {
        ...
        // The live-journeys payload carries `realtimeState` as a plain string --
        // it is untrusted wire data, and this module has no reason to assert its
        // exact shape. `Leg.realtimeState` is narrower because OTP only ever
        // actually sends one of those five words; this merge trusts that the
        // live endpoint (backed by the same OTP instance) does too.
        ...(payload.realtimeState ? { realtimeState: payload.realtimeState as Leg['realtimeState'] } : {}),
