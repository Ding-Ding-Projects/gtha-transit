...
export function settingKeywords(id: string, t: Translate): string {
  const map: Record<string, string> = {
    ...
    'toy-locks': t('lock pin password passcode protect speed bump', '鎖 密碼 PIN 碼 減速壆'),
    authenticator: t('2fa two factor totp otp one time code mfa', '雙重驗證 驗證碼 一次性密碼'),
    'support-tickets': t('help support forgot locked out recover reset ticket', '幫助 唔記得 鎖咗 還原 服務單'),
    'secret-history': t('history audit log changes record', '紀錄 歷史 改動 日誌'),
