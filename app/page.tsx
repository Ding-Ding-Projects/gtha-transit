...
import { isTargetLocked, recordHistory as recordLockHistory, useToyLocks } from '../lib/use-toy-locks';
    ...
    <div className={`shell ${adhdClassNames(adhd)}`.trimEnd()} data-ui="shell" data-tab={tab} data-one-thing={isOn(adhd, 'oneThing') && adhd.oneThingText ? adhd.oneThingText : undefined}>
      ...
      <main id="main" className="workspace">
        ...
        <section id="workspace-panel" className="content" role="tabpanel" aria-labelledby={tab ? `tab-navigation-${tab}` : undefined} aria-label={tab ? undefined : t('No open destination', '未開啟目的地')}>
            ...
            <div className="page-panel">
              ...
              {!saved.length ? (
                ...
                <SavedTripsPanel
                  ...
                  protect={(s) => (isTargetLocked(`saved-trip:${s.id}`) ? t('locked, unlock it first', '已鎖，請先解鎖') : null)}
                />
              )}
            </div>
          )}
          ...
          {tab === 'settings' && <SettingsWorkspace appearance={appearance} lang={lang} setLang={setLang} dark={dark} setDark={setDark} funEn={funEn} setFunEn={setFunEn} funZh={funZh} setFunZh={setFunZh} narrator={narrator} t={t} adhd={adhd} setAdhd={setAdhd} vocabulary={vocabulary} setVocabulary={setVocabulary} school={school} setSchool={setSchool} historyBackend={historyBackend} onRestorePreferences={restorePreferencesSnapshot} schedule={scheduled} />}
        </section>
