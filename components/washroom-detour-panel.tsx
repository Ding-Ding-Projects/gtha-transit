...
type DestinationReview = { place: Place; status: DestinationStatus };
type Facility = { name?: string | null; source?: string | null; availability?: 'confirmed-open' | 'closed' | 'unknown' | null };
type FacilityLeg = { itinerary?: unknown; timeToFacilitySeconds?: number; expectedArrival?: string | null; internalWalkingUnknown?: boolean };
type Continuation = { itinerary?: unknown; durationSeconds?: number; preservedTo?: Place; preservedVia?: Place[] };
type DetourResponse = {
  status?: string;
  scope?: string;
...
const destinationKey = (destinations: readonly Place[]) => destinations.map((place) => `${place.id}\u0000${place.name}\u0000${place.lat}\u0000${place.lon}`).join('\u0001');
...
export function washroomDetourRequest(position: WashroomPosition, remaining: readonly Place[], dateTime: string) {
  const currentPosition = { lat: position.lat, lon: position.lon };
  if (!remaining.length) return { currentPosition, dateTime, facilityOnly: true };
  return { currentPosition, dateTime, to: remaining.at(-1), via: remaining.slice(0, -1) };
}
...
export function WashroomDetourPanel({ position, destinations, t, onClose, onFollow }: WashroomDetourPanelProps) {
  ...
  const [review, setReview] = useState<DestinationReview[]>(() => destinations.map((place) => ({ place, status: 'remaining' })));
  ...
  useEffect(() => {
    setReview(destinations.map((place) => ({ place, status: 'remaining' })));
  }, [key]);
  ...
  const updateDestination = (id: string, status: DestinationStatus) => {
    setResult(null);
    setReview((items) => items.map((item) => item.place.id === id ? { ...item, status } : item));
  };
  ...
  const plan = async () => {
    ...
    if (!isFinitePosition(currentPosition)) {
      setError(t('Share a current location before planning a washroom detour.', '請先分享目前位置，先可以規劃洗手間繞道。'));
      return;
    }
    ...
    try {
      const response = await fetch('/api/plan-washroom-detour', {
        ...
        body: JSON.stringify(washroomDetourRequest(currentPosition, remaining, new Date().toISOString())),
  ...
  return <section className="washroom-detour-panel" aria-labelledby="washroom-detour-heading">
    ...
    <section className="washroom-detour-panel__position" aria-labelledby="washroom-position-heading">
      ...
      {isFinitePosition(currentPosition) ? <p><MapPin size={16} aria-hidden="true" /> {t('Current position is ready for a fresh route request.', '目前位置已準備好作新的路線請求。')}</p> : <div>
        <p>{t('A current position is required. Location is requested only after you choose this action.', '需要目前位置。只有你選擇此操作後先會請求位置。')}</p>
        <button type="button" onClick={requestLocation} disabled={geoBusy}><LocateFixed size={16} aria-hidden="true" /> {geoBusy ? t('Getting current location…', '取得目前位置中…') : t('Use current location', '使用目前位置')}</button>
      </div>}
    </section>
    ...
    <section className="washroom-detour-panel__destinations" aria-labelledby="washroom-destinations-heading">
      ...
      {review.length ? <ul>
        {review.map(({ place, status }) => <li key={place.id} className="washroom-detour-panel__destination">
          ...
          <div className="washroom-detour-panel__destination-actions">
            {status === 'remaining' ? <>
              <button type="button" onClick={() => updateDestination(place.id, 'visited')}><Check size={15} aria-hidden="true" /> {t('Mark visited', '標示為已到訪')}</button>
              <button type="button" onClick={() => updateDestination(place.id, 'removed')}><Trash2 size={15} aria-hidden="true" /> {t('Remove for this detour', '由今次繞道移除')}</button>
            </> : <button type="button" onClick={() => updateDestination(place.id, 'remaining')}><RefreshCw size={15} aria-hidden="true" /> {t('Restore to continuation', '還原到後續行程')}</button>}
          </div>
        </li>)}
    ...
    <div className="washroom-detour-panel__actions">
      <button type="button" onClick={plan} disabled={busy || !isFinitePosition(currentPosition)}><Navigation size={16} aria-hidden="true" /> {busy ? t('Planning washroom detour…', '規劃洗手間繞道中…') : remaining.length ? t('Plan facility and continuation', '規劃設施與後續行程') : t('Plan facility only', '只規劃去設施')}</button>
    </div>
    ...
    {result && <section className="washroom-detour-panel__result" aria-live="polite" aria-labelledby="washroom-result-heading">
      ...
      <section className="washroom-detour-panel__leg" aria-labelledby="washroom-facility-leg-heading">
        ...
        {facilityJourney ? <><p>{t('ETA', '預計時間')}: {displaySeconds(facilityLeg?.timeToFacilitySeconds, t)}</p><p>{facilityLeg?.internalWalkingUnknown ? t('Indoor walking is not available in this route result.', '呢個路線結果未有室內步行資料。') : null}</p><button type="button" onClick={followFacility} disabled={result.facility?.availability !== 'confirmed-open'}>{t('Follow facility', '跟隨去設施')}</button></> : <p>{t('A facility leg is not available.', '未有去設施路段。')}</p>}
      </section>
