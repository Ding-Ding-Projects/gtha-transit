...
Latest confirmed frontend and backend source: `b92de7cc79e1d57d06db82979f07de825611003e`, frontend built `2026-09-05T10:44:38.044Z`. Public HTML serves `no-transform` and no longer includes the injected analytics script. Actual browser font loading at the earlier `7bfa8b3` confirms Manrope at weights 400 through 800; the new browser matrix is in progress. Public required-Line-5 planning returns two complete itineraries, with exact `ttc:5` transit legs and a shortest extra duration of 860 seconds. The public washroom POST initially returned 405 because the proxy method allowlist omitted it; a narrow proxy repair and real HTTP regression now pass locally, with its deployment pending completion of the pinned browser run.

Current integration adds ordered destinations, intermediate insertion preserving the final destination, pointer and keyboard reordering, saved/shared order, less-transfer-waiting selection, dated stop-route enrichment and trip/vehicle follower entry points. It also wires backend required-line and washroom-detour adapters. Root tests: 134 passed. Backend's own `npm test`: 50 passed. Type checking passed. These are local checks, not built UI or production proof. Do not deploy the new frontend before generating the expanded stop/pattern indexes and deploying its backend. Required-line selection and urgent washroom UI remain unfinished. Follower layout, focus return and real browser interaction still need verification.

The browser-based GTHA planner is live at https://toronto-transit.org, with the owner-managed tunnel and a private LAN fallback. Routing API source `acced3d` and its expanded source indexes are deployed. The index contains 38526 stops, 1015 source routes and 4070 representative patterns from 16218863 stop-time rows. Of 575587 declared trips, 575517 have stop times; the 70 empty HSR trips are explicitly excluded from patterns. Exact source archive checksums and exclusion arithmetic accompany the generated indexes. Compact-query normalization is deployed to the map service too. Map dataset revision `199cc9933f670a7fc1c33099a18891a53f5d187212c0041ce2b95ac6bafd381c` was generated by renderer `fdd0ec2a716f17cc4380ee2096b2974e6703671b` and validated with `734a1899583c506cb9b97880eea77c29edd66a03`.

Backend probes returned 10 ordinary Eglinton-to-Union itineraries and one complete itinerary through Bloor-Yonge. Selected Eglinton platform `ttc:13795` exposes its official route and an agency-qualified washroom with unknown hours. Additional Sep5 noon probes with the waiting preference returned Kipling-Finch 10, Don Mills-Yorkdale 8, Scarborough Centre-Union 10 and Brampton Gateway-Pearson 9 options. These are routing/API observations, not physical travel evidence. Required-line coordinate anchors failed at platform edges; a stop-identity repair is being verified. The urgent washroom UI and dwell/ETA refinements are not yet deployed.
