...
export async function planWashroomDetour(input, { planWithOtp, facilityRegistry, stopIndex = null, now = () => Date.now(), deadlineMs = MAX_WASHROOM_DETOUR_DEADLINE_MS } = {}) {
  ...
  if (!current) return noResult("CURRENT_POSITION_UNRESOLVED", "An explicit current coordinate or uniquely verified agency-qualified stop is required before a washroom detour can be planned.");
  const destination = point(input?.to);
  if (!destination) return noResult("DESTINATION_UNRESOLVED", "The remaining destination needs valid coordinates before a washroom detour can be planned.");
  const via = normalizedVia(input?.via);
  if (!via) return noResult("VIA_UNRESOLVED", "Each remaining stop must have valid coordinates before a washroom detour can be planned.");
  const dateTime = validDateTime(input?.dateTime);
  if (!dateTime) return noResult("DATETIME_UNRESOLVED", "A timestamp with an explicit offset is required before availability can be checked at facility arrival.");
  ...
  if (!pool.length) {
    ...
    return noResult(publishedButUnroutable ? "FACILITY_COORDINATES_UNAVAILABLE" : "NO_CONFIRMED_AVAILABILITY", publishedButUnroutable ? "No facility with published hours has verified routing coordinates. Municipal facility coordinates are not inferred." : "No facility has published hours that can be confirmed for automatic washroom routing.", { locationSource: current.source });
  }
